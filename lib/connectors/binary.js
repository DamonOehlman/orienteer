...
OrientSocket.prototype.connect = function(connection, callback) {
    ...
    this.once('connect', function() {
        ...
        socket.once('data', function(buffer) {
            ...
            // if we have a target db, then do a db open
            if (this.targetDb) {
                commandData.command = 'dbOpen';
                commandData.dbname = connection.dbname;
                commandData.user = connection.dbuser;
                commandData.password = connection.dbpass;
                commandData.type = connection.dbtype || 'document';
            }
            ...
            socket.send(connection.sessionId, commandData, function(err, response) {
                ...
                socket.sessionId = connection.sessionId = response.sessionId;
...
OrientSocket.prototype._buildRequest = function(sessionId, data) {
    ...
    // if we have a 0 length buffer, report an error
    if (buffer.length === 0) return new OrientError('invalid-buffer');

    // now that the buffer has been created, write the data
    debug('allocated ' + buffer.length + ' for write buffer, writing data');

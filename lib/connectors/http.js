var debug = require('debug')('orienteer-http'),
    ...
    OrientError = errors.OrientError,
    OrientServerError = errors.OrientServerError,
    OrientNoClassError = errors.OrientNoClassError,
    methodMap = {
        get:  'GET',
        put:  'PUT',
        post: 'POST',
    ...
    reStatusOK = /^[23]\d{2}$/,
    reStatusError = /^5\d{2}$/,
    reMissingClass = /^invalid\sclass/i,
    ...
    prepareRequest(connection, data).get(
        ...
        function(err, response) {
            ...
            if (err && reMissingClass.test(err.message)) {
                return callback(new OrientNoClassError(connection.dbname, data.name));
            }

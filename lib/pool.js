...
OrientSocket.prototype.connect = function(connection, callback) {
    var socket = this, buffer;

    // bind the error handler
    this.once('error', callback);

    // if we capture a close event, then return an error condition
    this.once('close', function handleClose() {
    ...
    this.once('connect', function() {
        ...
        // once we receive the socket response
        socket.once('response', function handleResponse(command, data) {
            // unbind the close handler
        ...
        socket.write(buffer = connection._buildRequest({
            command: 'connect',
            user: connection.user,
            password: connection.password
        }));

        debug('sent connection command (' + buffer.length + ' bytes)', buffer);
    });

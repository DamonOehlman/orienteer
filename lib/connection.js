...
Connection.prototype.as = function(username) {
    return new Connection(this).user(username);
};
...
Connection.prototype.getDbUser = function(targetDb, targetName) {
    ...
    targetName = targetName || this.currentUser;
...
Connection.prototype.send = function(data, callback) {
    ...
    data = _.extend({}, data, this.getDbUser(this.dbname, data.username));

    // get the connection pool to send the data across the wire
    return this.connector.send(this, data, callback);
};
...
Change the currently selected user to the specified user name
*/
Connection.prototype.user = function(username) {
